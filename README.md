...
New portal registrations always require administrator approval. Email verification is normally required as well, but production can temporarily use approval-only onboarding with `REQUIRE_EMAIL_VERIFICATION=false` until a church-controlled sending domain is available. Pending users may sign in only to view their setup status; they receive no member or operational permissions. Transactional email and the temporary mode are documented in `SITE_DEPLOYMENT.md`.
