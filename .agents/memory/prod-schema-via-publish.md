...
- As of 2026-07-10 the app has never been published (user plans to publish only after migrating to a personal repo), so prod queries return PRODUCTION_DATABASE_ERROR. Post-publish verification queries: `SELECT extname FROM pg_extension WHERE extname='pg_trgm'` and `SELECT indexname FROM pg_indexes WHERE tablename='members' AND indexname LIKE '%trgm%'` (expect first/last/full_name trgm idx).
