...
function recommendedView(user: SafeUser | null): DocumentationView {
  ...
  if (roles.some((role) => role === "finance_committee" || role === "treasurer")) return "finance";
...
export function useDocumentationView(user: SafeUser | null) {
  const [view, setViewState] = useState<DocumentationView>(() => storedView() ?? recommendedView(user));
  useEffect(() => {
    if (!storedView()) setViewState(recommendedView(user));
  }, [user]);
